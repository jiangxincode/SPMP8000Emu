...
impl FunctionTable {
    ...
    pub fn setup_in_memory(&self, memory: &mut Memory) -> Result<()> {
        ...
        for (offset, svc_num) in native_funcs {
            let addr = FUNC_TABLE_BASE + offset;
            // Write SVC instruction: 0xEF000000 | svc_num
            let svc_instr = 0xEF000000u32 | svc_num;
            memory.write_u32(addr, svc_instr)?;
        }
...
mod tests {
    ...
    fn test_function_table_setup() {
        ...
        // Verify some SVC instructions were written
        let val = memory.read_u32(FUNC_TABLE_BASE + NativeFuncIndex::DiagPrintf as u32).unwrap();
